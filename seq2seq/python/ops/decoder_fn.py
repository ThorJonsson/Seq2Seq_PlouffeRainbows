...
def simple_regression_decoder_fn_inference(output_fn, encoder_state,
                                start_of_sequence_id, end_of_sequence_id,
  ...
  """
  with ops.name_scope(name, "simple_decoder_fn_inference",
                      [output_fn, encoder_state, batch_size, num_features
                       maximum_length, dtype]):
    start_of_sequence_id = ops.convert_to_tensor(start_of_sequence_id, dtype)
    end_of_sequence_id = ops.convert_to_tensor(end_of_sequence_id, dtype)
    maximum_length = ops.convert_to_tensor(maximum_length, dtype)
    if output_fn is None:
      output_fn = lambda x: x # TODO Explain
  ...
  """
    with ops.name_scope(name, "simple_decoder_fn_inference",
      ...
      if cell_output is None:
        # invariant that this is time == 0
        next_input = array_ops.ones([batch_size,num_features], dtype=dtype) * start_of_sequence_id
        done = array_ops.zeros([batch_size, num_features], dtype=dtypes.bool)
        cell_state = encoder_state
      ...
      # if time > maxlen, return all true vector
          ...
          lambda: done)
      return (done, cell_state, next_input, context_state)
  return decoder_fn
